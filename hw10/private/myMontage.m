...
%info=[imData.info];	% This line breaks sometimes!
%width=[info.Width];
%height=[info.Height];
for i=1:length(imData)
	width(i)=imData(i).info.Width;
	height(i)=imData(i).info.Height;
	nbits(i)=imData(i).info.BitDepth;
end
if any(diff(nbits))
	minBit=min(nbits);
	if minBit~=8, error('Min BitDepth is not 8!'); end
	for i=1:length(imData)
