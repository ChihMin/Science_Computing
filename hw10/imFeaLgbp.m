...
%				opt.orientation = 8;		%%  Gabor Orientation
%				opt.scale = 5;			%%  Gabor Scale
%				opt.mask = [32 32];		%%  Gabor Mask
%				opt.blockSize = 8;		%%  LBP Blocksize
...
lbpOpt=imFeaLbp('defaultOpt');
lbpOpt.patchSideCount=opt.blockSize;
%% Transform  Gabor feature to LBP ¡÷ LGBP
for u = 1:opt.orientation
	for v = 1:opt.scale
		[tmp, LGBP_im{u,v}] = imFeaLbp(gabor_output{u,v}, lbpOpt);
