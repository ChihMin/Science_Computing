...
if ischar(im) && strcmpi(im, 'defaultOpt')	% Set the default options
	...
	patchHist.weight=2.^[0, 3, 5, 1, -inf, 6, 2, 4, 7];	% Used by LBP2.m
...
for i=1:size(lbpPatch,2)
	...
	patchHist(:,i)=patchHist(:,i)/max(patchHist(:,i));	% Used by LBP2.m. But is this normalization necessary?
end
patchHist=patchHist(:);
...
if showPlot
	...
	for i=1:patchSideCount(1)
		for j=1:patchSideCount(2)
			...
			temp=hist(lbpPatch(:, k), 256); temp=temp/max(temp);
			bar(0:255, temp); axis tight
		end
	end
end
